...
public class MaxTwoEvents {
    public static void main(String[] args) {
        MaxTwoEvents ans = new MaxTwoEvents();
        System.out.println(ans.maxTwoEvents(new int[][]{{1, 3, 2}, {4, 5, 2}, {2, 4, 3}}));
    }
...
//        // Step 2: Create the suffix array for the maximum event value from each event onward
//        int[] suffixMax = new int[n];
//        suffixMax[0] = events[0][2];  // Initialize the last event's value
//
//        // Populate the suffixMax array
//        for (int i = 0; i < n - 1; i++) {
//            suffixMax[i] = Math.max(events[i][2], suffixMax[i + 1]);
